...
float4 main(float3 worldPos : POSITION, float3 normal : NORMAL, float2 coord : TEXCOORD) : SV_TARGET
{
    ...
    float satVR = saturate(dot(R, V));
    float satNL = saturate(-dot(L, N));
    
    float maxRf = max(max(kSpecular.r, kSpecular.g), kSpecular.b);
    float3 fresnel = kSpecular + (1 - kSpecular) * pow(1.0 - satNL, 5);
	
    float3 colour = ((colorDiffuse * (1.0-maxRf)) + ((kShininess + 2.0) / 2) * fresnel * pow(satVR, kShininess)) * lightColor * satNL + ambientColor * colorDiffuse;
